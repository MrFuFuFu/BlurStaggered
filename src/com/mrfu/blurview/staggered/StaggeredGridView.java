...
package com.mrfu.blurview.staggered;
...
//    }
    ...
    public int getFirstVisiblePosition(){
    	return Math.max( 0, mFirstPosition - 1);// 1 replace getHeaderViewsCount();
    }
...
//        for (ArrayList<Integer> map : this.mColMappings) {
        for (HashSet<Integer> map : this.mColMappings) {
            sb.append("COL" + col + ":");
            sb.append(' ');
            for (Integer i : map) {
                sb.append(i);
...
//                mColMappings.add(colMap.values);
    ...
    private class AdapterDataSetObserver extends DataSetObserver {
        ...
        public void onChanged() {
            ...
            if (lazyload) {
            	if (isUnLock) {//如果是单个item，则刷新全部界面，否则正常处理
            		removeAllViews();
            		dealChild();
            		isUnLock = false;
				}else {
